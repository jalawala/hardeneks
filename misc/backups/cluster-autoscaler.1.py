    ...
    for deployment in deployments:
        if deployment.metadata.name == "cluster-autoscaler":
            ...
            if cluster_version in ca_image_version:
                print_console_message(None, None, "green", func_name, versions + " match", docs_link)
                return True
            else:
                print_console_message(None, None, "red", func_name, versions + " do not match", docs_link)
                return False
                    
    print_console_message(None, None, "red", func_name, "Kubernetes Cluster Autoscaler is not deployed in the cluster", docs_link)
    return False
        ...
        if deployment.metadata.name == "cluster-autoscaler":
            ...
            for item in ca_command:
                if 'node-group-auto-discovery' in item:
                    ...
                    print_console_message(None, None, "green", func_name, "Kubernetes Cluster Autoscaler is configured with Auto Discovery Mode", docs_link)
                    return True
                        
            print_console_message(None, None, "red", func_name, "Kubernetes Cluster Autoscaler is not configured with Auto Discovery Mode", docs_link)
            return False
...
def ensure_cluster_autoscaler_has_three_replicas(resources: Resources):
    ...
    for deployment in deployments:
        if deployment.metadata.name == "cluster-autoscaler":
            ...
            if ca_replicas == 3:
                print_console_message(None, None, "green", func_name, "Kubernetes Cluster Autoscaler has three replicas", docs_link)
                return True
            else:
                print_console_message(None, None, "red", func_name, "Kubernetes Cluster Autoscaler does not have three replicas", docs_link)
                return False
...
def use_separate_iam_role_for_cluster_autoscaler(resources: Resources):
    ...
    func_name = sys._getframe().f_code.co_name
    docs_link = "https://aws.github.io/aws-eks-best-practices/cluster-autoscaling/#employ-least-privileged-access-to-the-iam-role"
    
    ca_deploy = client.AppsV1Api().read_namespaced_deployment(name="cluster-autoscaler", namespace="kube-system")
    sa = ca_deploy.spec.template.spec.service_account_name
    ...
    if 'eks.amazonaws.com/role-arn' in sa_data.metadata.annotations.keys():
        print_console_message(None, None, "green", func_name, "cluster-autoscaler deployment uses a dedicated IAM Role (IRSA)", docs_link)
        return True
    else:
        print_console_message(None, None, "red", func_name, "cluster-autoscaler deployment does not use a dedicated IAM Role (IRSA)", docs_link)
        return False
...
def employ_least_privileged_access_to_the_IAM_role(resources: Resources):
    ...
    if 'eks.amazonaws.com/role-arn' in sa_data.metadata.annotations.keys():
        ...
        if administratorAccess is None and  leastPrivelegedAccess:
            print_console_message(None, None, "green", func_name, "cluster-autoscaler has least privileged access to the IAM role", docs_link)
            return True            
        else:
            print_console_message(None, None, "red", func_name, "cluster-autoscaler does not have least privileged access to the IAM role", docs_link)
            return False            
...
def use_managed_nodegroups(resources: Resources):
    ...
    if len(selfmnglist) == 0:
        descriptionMessage = "cluster has only managed node groups :{}".format(eksmnglist)
        print_console_message(None, None, "green", func_name, descriptionMessage, docs_link)
    else:
        descriptionMessage = "cluster has self managed node groups :{}".format(selfmnglist)
        print_console_message(None, None, "red", func_name, descriptionMessage, docs_link)
        return False
...
def ensure_uniform_instance_types_in_nodegroups(resources: Resources):
    ...
    if not isNonUniformNodegroupsExists:
        descriptionMessage = "cluster has only unfirm instance types in the node groups"
        print_console_message(None, None, "green", func_name, descriptionMessage, docs_link)
    else:
        print_console_message(None, None, "red", func_name, descriptionMessage, docs_link)
        return False
