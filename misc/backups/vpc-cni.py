...
def deploy_vpc_cni_managed_add_on(resources: Resources):
    ...
    try:
        ...
        print_console_message(None, None, "green", func_name, "VPC CNI is a Managed Add-On", docs_link)
        #pprint("vpccni_add_on={}".format(vpccni_add_on))
    except Exception as exc:
        #print(f"[bold][red]{exc}")
        print_console_message(None, None, "red", func_name, "VPC CNI is Not a Managed Add-On", docs_link)
        return False        
...
def use_separate_iam_role_for_cni(resources: Resources):
    ...
    func_name = sys._getframe().f_code.co_name
    docs_link = "https://aws.github.io/aws-eks-best-practices/networking/vpc-cni/#use-separate-iam-role-for-cni"
    ...
    if 'eks.amazonaws.com/role-arn' in sa_data.metadata.annotations.keys():
        print_console_message(None, None, "green", func_name, "aws-node daemonset uses a dedicated IAM Role (IRSA)", docs_link)
        return True
    else:
        print_console_message(None, None, "red", func_name, "aws-node daemonset does not use a dedicated IAM Role (IRSA)", docs_link)
        return False
        

def monitor_IP_adress_inventory(resources: Resources):
    func_name = sys._getframe().f_code.co_name
    docs_link = "https://aws.github.io/aws-eks-best-practices/networking/vpc-cni/#monitor-ip-address-inventory"
    ...
    if totalAvailableIpAddressCount > 5000:
        print_console_message(subnetsList['Subnets'], "IP", "green", func_name, descriptionMessage, docs_link)
    else:
        print_console_message(subnetsList['Subnets'], "IP", "red", func_name, descriptionMessage, docs_link)
        return False
...
def use_dedicated_and_small_subnets_for_cluster_creation(resources: Resources):
    func_name = sys._getframe().f_code.co_name
    docs_link = "https://docs.aws.amazon.com/eks/latest/userguide/network_reqs.html"
    ...
    descriptionMessage = "Cluster Subnet CIDE Size is larger than /28"
    if not is_cluster_subnet_cidr_size_big:
        print_console_message(subnetsList['Subnets'], "CIDR", "green", func_name, descriptionMessage, docs_link)
    else:
        print_console_message(subnetsList['Subnets'], "CIDR", "red", func_name, descriptionMessage, docs_link)
        return False
